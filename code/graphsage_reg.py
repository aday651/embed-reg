...
#################################################################
# Modified version of layer/graphsage.py from Stellargraph
...
from tensorflow.keras.layers import Lambda, Dropout, Reshape, LeakyReLU, Dense
...
from stellargraph.layer.misc import deprecated_model_function
from stellargraph.connector.neo4j.mapper import Neo4jGraphSAGENodeGenerator


class GraphSAGEAggregatorReg(Layer):
    """
    ...
    def get_config(self):
        ...
        config = {
            ...
            "activity_regularizer": regularizers.seralize(self.activity_regularizer)
        }
...
class MeanAggregator(GraphSAGEAggregatorReg):
    """
...
class MaxPoolingAggregator(GraphSAGEAggregatorReg):
    """
...
class MeanPoolingAggregator(GraphSAGEAggregatorReg):
    """
...
class AttentionalAggregator(GraphSAGEAggregatorReg):
    """
...
class GraphSAGEReg:
    ...
    def __init__(
        ...
        self.trainable_ip = trainable_ip
        ...
        activities = [None] * (self.max_hops - 1) + [activity_regularizer]
        ...
        self._aggs = [
            self._aggregator(
                ...
                activity_regularizer=activities[layer]
            )
    ...
    def __call__(self, xin: List):
        ...
        return (
            ...
            if len(h_layer) == 1
            else ([self._normalization(xi) for xi in h_layer[:-1]] + [h_layer[-1]])
        )
    ...
    def _node_model(self):
        ...
        if self.trainable_ip:
            x_out = Dense(self.layer_sizes[-1], use_bias = False)(self(x_inp))
        else:
            x_out = self(x_inp)

...
#################################################################
# Modified version of layer/gcn.py from Stellargraph
...
from tensorflow.keras.layers import Input, Layer, Lambda, Dropout, Reshape, ActivityRegularization

from stellargraph.mapper import FullBatchGenerator, ClusterNodeGenerator
from stellargraph.layer.misc import SqueezedSparseConversion, deprecated_model_function, GatherIndices
from stellargraph.layer.preprocessing_layer import GraphPreProcessingLayer


class GraphConvolutionReg(Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(GraphConvolutionReg, self).__init__(
...
class GCNReg:
    ...
    def __init__(
        ...
        self._layers.append(ActivityRegularization(l2=self.reg_weight))
    ...
    def __call__(self, x):
        ...
        for layer in self._layers:
            if isinstance(layer, GraphConvolutionReg):
